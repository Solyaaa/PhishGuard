...
def check_password():
    try:
        ...
        if not password:
            return jsonify({"error": "Пароль не надано"}), 400


        result = detector.check_password_leak(password)


        security_logger.info(f"Перевірка пароля від IP: {request.remote_addr}")

        return jsonify(result)

    except Exception as e:
        logger.exception("Помилка при перевірці пароля")
        return jsonify({"error": "Внутрішня помилка сервера"}), 500
